...
const defineSpfWin = {
    ...
    watch: {
        ...
        tabActive: {
            handler(nv, ov) {
                console.log(this.tabList, this.tab.dftItem, this.tab.active);
                let is= this.$is,
                    ta = this.tabActive;
                if (!is.string(this.tab.active)) {
                    //首次写入
                    if (!is.string(ta)) {
                        this.tab.active = '';
                    } else {
                ...
                //已初始化后
                if (this.isLegalTabKey(ta)) {
                    //如果外部制定的 active key 存在且合法 则写入 tab.active
                    this.tab.active = ta;
                } else {
                    //否则使用 tabItemList[0].key 作为 tab.active
                    let tbl = this.tabItemList;
                    if (is.array(tbl) && tbl.length>0) {
                        this.tab.active = tbl[0].key;
                    } else {
                        this.tab.active = '';
                    }
                }
    ...
    methods: {
         ...
         */
        winMinimize(event) {
            if (this.winType==='inside' || this.minimized || this.winKey==='') return false;
            this.minimized = true;
        ...
        winMaximize(event) {
            if (this.winType==='inside' || this.winKey==='') return false;
...
defineSpfWin.template = `<div :class="styComputedClassStr.root" :style="styComputedStyleStr.root"><div class="win-titbar flex-x" v-drag-move:xy="$this"><spf-icon v-if="icon !== '' && icon !== '-empty-'" :icon="icon" :shape="iconShape" :spin="spin" :color="iconColor" v-bind="iconParams"></spf-icon><div class="win-title flex-1 flex-x">新窗口</div><div class="win-titctrl flex-x flex-x-end"><slot name="titctrl"></slot></div><template v-if="winType !== 'inside'"><spf-button v-if="minimizable" icon="arrow-downward" effect="popout" stretch="square" no-gap @click="winMinimize"></spf-button><spf-button v-if="maximizable" :icon="maximized ? 'fullscreen-exit' : 'fullscreen'" effect="popout" stretch="square" no-gap @click="winMaximize"></spf-button><spf-button v-if="closeable" icon="close" type="danger" effect="popout" stretch="square" no-gap @click="winClose"></spf-button></template></div><div v-if="tabList.length>1 && tabItemList.length>1" class="win-tabbar flex-x flex-x-center"><spf-tabbar :value="tab.active" :tab-list="tabItemList" :size="tabbarSize" v-bind="tabbarParams" @tab-active="whenTabActive"></spf-tabbar></div><div class="win-body flex-1 flex-y flex-x-stretch"><template v-if="$is.plainObject(tabActiveItem) && !$is.empty(tabActiveItem)"><slot v-if="tabActiveItem.component === ''" :name="'tab-'+tabActiveItem.key" :tab="tab" :win="$this"></slot><component v-else :is="'spf-'+tabActiveItem.component" v-bind="tabActiveItem.compParams"></component></template><slot></slot></div><div class="win-ctrlbar flex-x flex-x-end"><slot name="winctrl-extra"></slot><div class="flex-1"></div><slot name="winctrl"></slot><spf-button v-if="cancelButton" :icon="cancelIcon" :label="cancelLabel" effect="popout" type="danger" debounce fullfilled :fullfilled-when="winCanceled" @click="$emit('cancel')"></spf-button><spf-button v-if="confirmButton" :icon="confirmIcon" :label="confirmLabel" effect="fill" type="primary" debounce fullfilled :fullfilled-when="winConfirmed" @click="$emit('confirm')"></spf-button></div></div>`;
