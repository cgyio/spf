...
const defineSpfMenu = {
        ...
        menus: {
            handler(nv, ov) {
                if (this.inited!==true) {
                    ...
                    //已初始化过 仅通过 v-model 同步外部 menus 与 currentMenus
                    //此处不做操作
                    //!! 所有对 menus 的状态修改都应通过 spf-menu 组件，
                    //!! 不应手动修改外部的 menus 菜单参数值
                    //this.updateMenus();
                }
            },
            //创建时立即执行
    ...
    methods: {
        ...
        async toggleCompact() {
            ...
            this.$each(menus, (mi,i) => {
                ...
                if (compact!==true) {
                    ...
                    //如果此菜单处于 active 状态，则为一级菜单增加 active 标记，用于在横向折叠时，也能显示哪个一级菜单的子菜单处于 active 状态
                    if (isac(mi)) {
        ...
        async whenMenuItemClick(keyChain=[], idxChain=[]) {
            ...
            //取消原 active 菜单项
            let aidxc = this.activeMenuIdxChain;
            if (is.array(aidxc) && aidxc.length>0) {
                let ami = this.getCurrentMenuByIdxChain(aidxc);
                if (isim(ami)) {
                    //原 active 菜单项 取消 active
                    this.$set(ami.params, 'active', false);
                }
                //取消缓存的 active 菜单项的 idxChain
                this.activeMenuIdxChain.splice(0);
            }
            //设置当前菜单项的 active = true
            this.$set(mi.params, 'active', true);
            //保存此菜单项为 active 菜单项
            this.activeMenuIdxChain.push(...idxChain);
...
defineSpfMenu.template = `<spf-block v-bind="$attrs" :size="size" :grow="grow" :scroll="scroll" v-slot="{styProps, cntParams}"><template v-if="manual"><slot v-bind="{styProps, cntParams}"></slot></template><template v-if="!manual && inited && currentMenus.length>0"><template v-for="(menui, midx) of currentMenus"><spf-menu-item v-if="!menui.params.hide" :key="'spf-menu-item-'+midx" :size="styProps.size" :color="color" :background="background" :border="border" :label="menui.label" :icon="menui.icon" :sub="menui.sub" v-bind="menui.params" :menu-comp="$this" :compact="compact" @menu-item-toggle-collapse="whenMenuItemToggleCollapse" @menu-item-click="whenMenuItemClick"></spf-menu-item></template></template></spf-block>`;
...
const defineSpfMenuItem = {
    ...
    computed: {
        ...
        //根据 compact 状态，计算 padding-left|right 这两个值一定相等
        /*menuCompactPadding() {
            //仅对一级菜单进行计算
            if (!this.compact || this.menuDeepLevel!==1) return '';
            ...
            let is = this.$is,
                ...
                //菜单尺寸 normal|m|...
                msz = this.size,
                //横向折叠状态下，菜单栏宽度默认是  cssvar.size.bar.m 
                mw = sv(msz, 'bar'),
                //菜单图标尺寸
                isz = sv(msz, 'icon'),
                //默认 padding
        ...
        menuHeaderParams() {
            ...
            //处理 一级菜单 且 compact == true 横向折叠时，如果 active == true 则增加 active 标记
            if (this.menuDeepLevel===0 && this.compact && this.active) rtn.active = true;
