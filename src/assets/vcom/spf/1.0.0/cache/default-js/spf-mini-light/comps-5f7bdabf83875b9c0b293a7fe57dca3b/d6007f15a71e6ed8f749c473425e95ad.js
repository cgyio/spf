...
const defineSpfWin = {
    ...
    watch: {
        //外部指定的 loading 状态
        loading(nv, ov) {
            this.display.loading = this.loading;
        },
        //指定了外部 tab-active
        tabActive: {
            handler(nv, ov) {
    ...
    methods: {
         ...
         */
        winMinimize(event) {
            if (this.winType==='inside' || this.display.minimized || this.winKey==='') return false;
        ...
        winMaximize(event) {
            ...
            this.display.maximized = !this.display.maximized;
        ...
        winLoading(loading=null) {
            ...
            if (!is.boolean(loading)) {
                ...
                this.display.loading = !this.display.loading;
            } else {
                //直接指定 loading 状态
                this.display.loading = loading;
            }
            //事件
            return this.$emit('loading', this.display.loading);
        },
...
defineSpfWin.template = `<div :class="styComputedClassStr.root" :style="styComputedStyleStr.root"><div class="win-titbar flex-x" v-drag-move:xy="$this"><spf-icon v-if="icon !== '' && icon !== '-empty-'" :icon="icon" :shape="iconShape" :spin="loading ? true : spin" :color="iconColor" v-bind="iconParams"></spf-icon><div class="win-title flex-1 flex-x">新窗口</div><div class="win-titctrl flex-x flex-x-end"><slot name="titctrl"></slot></div><template v-if="winType !== 'inside'"><spf-button v-if="minimizable" icon="arrow-downward" effect="popout" stretch="square" no-gap @click="winMinimize"></spf-button><spf-button v-if="maximizable" :icon="display.maximized ? 'fullscreen-exit' : 'fullscreen'" effect="popout" stretch="square" no-gap @click="winMaximize"></spf-button><spf-button v-if="closeable" icon="close" type="danger" effect="popout" stretch="square" no-gap @click="winClose"></spf-button></template></div><div v-if="tabList.length>1 && tabItemList.length>1" class="win-tabbar flex-x flex-x-center"><spf-tabbar :value="tab.active" :tab-list="tabItemList" :size="tabbarSize" v-bind="tabbarParams" @tab-active="whenTabActive"></spf-tabbar></div><div class="win-body flex-1 flex-y flex-x-stretch"><template v-if="$is.plainObject(tabActiveItem) && !$is.empty(tabActiveItem)"><slot v-if="tabActiveCompName === ''" :name="'tab-'+tabActiveItem.key" :tab="tabActiveItem" :win="$this"></slot><component v-else :is="tabActiveCompName" v-bind="tabActiveItem.compParams"></component></template><slot></slot></div><div class="win-ctrlbar flex-x flex-x-end"><slot name="winctrl-extra"></slot><div class="flex-1"></div><slot name="winctrl"></slot><spf-button v-if="cancelButton" :icon="cancelIcon" :label="cancelLabel" effect="popout" type="danger" debounce fullfilled :fullfilled-when="winCanceled" @click="$emit('cancel', $this)"></spf-button><spf-button v-if="confirmButton" :icon="confirmIcon" :label="confirmLabel" effect="fill" type="primary" debounce fullfilled :fullfilled-when="winConfirmed" @click="$emit('confirm', $this)"></spf-button></div><div v-if="display.loading" class="win-loading flex-x flex-x-center flex-y-center"><spf-icon icon="spinner-spin" size="huge" type="primary" spin></spf-icon></div></div>`;
