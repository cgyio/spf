            ...
            switch: {
                root: {
                    '!autoExtra.disabled': {
                        ...
                        'autoExtra.hoverable && stage.press': '.pressed',
                        'fullfilledWhen': '.fullfilled'
                    },
                    'astitle #style': {
                        paddingLeft: '{{$ui.sizeValMul(sizePropVal, 0.5)}}',
                        paddingRight: '{{$ui.sizeValMul(sizePropVal, 0.5)}}',
                    },
                },
                ...
                icon: {
                    ...
                    '!isEmptyIcon': {
                        '["str","key"].includes(sizePropType)!==true #style': 'font-size: {{$ui.sizeValMul(sizePropVal, 0.5)}};',
                        'astitle #props': {
                            size: '{{size}}',
                        },
                    },
                ...
                label: {
                    'incell #class': '.flex-1 flex-x flex-x-center',
                    'astitle #style': {
                        fontWeight: 'bold',
                        fontSize: '{{$cssvar.size.fs[autoSizeShift("l1","fs")]}}',
                        marginLeft: '{{$ui.sizeValMul(sizePropVal, 0.25)}}',
                    },
                },
...
defineSpfBtn.template = `<div :class="autoComputedStr.root.class" :style="autoComputedStr.root.style" @click="whenBtnClick" @mouseenter="whenMouseEnter" @mouseleave="whenMouseLeave" @mousedown="whenMouseDown" @mouseup="whenMouseUp"><spf-icon v-if="!iconRight && !isEmptyIcon" v-bind="autoComputed.icon.props" :root-class="autoComputedStr.icon.class" :root-style="autoComputedStr.icon.style"></spf-icon><spf-btn v-if="iconRight && closeable" v-bind="autoComputed.btn.props" :root-class="autoComputedStr.btn.class" :root-style="autoComputedStr.btn.style" @click="whenClose"></spf-btn><template v-if="!isSquare"><label v-if="!isEmptyLabel" :class="autoComputedStr.label.class" :style="autoComputedStr.label.style">{{label}}</label><template v-else><slot v-bind="{autoParams}"></slot></template></template><spf-btn v-if="!iconRight && closeable" v-bind="autoComputed.btn.props" :root-class="autoComputedStr.btn.class" :root-style="autoComputedStr.btn.style" @click="whenClose"></spf-btn><spf-icon v-if="iconRight && !isEmptyIcon" v-bind="autoComputed.icon.props" :root-class="autoComputedStr.icon.class" :root-style="autoComputedStr.icon.style"></spf-icon></div>`;

if (defineSpfBtn.computed === undefined) defineSpfBtn.computed = {};
defineSpfBtn.computed.profile = function() {return {};}
...
defineSpfBar.template = `<div :class="autoComputedStr.root.class" :style="autoComputedStr.root.style" @click="whenBarClick"><template v-if="innerContent"><spf-bar v-if="!iconRight && !isEmptyIcon" v-bind="autoComputed.iconbar.props" :icon-props="autoComputed.icon.props" :icon-class="autoComputedStr.icon.class" :icon-style="autoComputedStr.icon.style"></spf-bar><spf-bar v-if="iconRight && !isEmptyBtn" v-bind="autoComputed.btnbar.props" :btn-props="autoComputed.btn.props" :btn-class="autoComputedStr.btn.class" :btn-style="autoComputedStr.btn.style" @suffix-btn-click="$emit('suffix-btn-click')"></spf-bar><div :class="autoComputedStr.cnt.class" :style="autoComputedStr.cnt.style"><slot v-bind="{ autoParams, cntProps: autoComputed.cnt.props, cntClass: autoComputed.cnt.class, cntStyle: autoComputed.cnt.style, }"></slot></div><spf-bar v-if="!iconRight && !isEmptyBtn" v-bind="autoComputed.btnbar.props" :btn-props="autoComputed.btn.props" :btn-class="autoComputedStr.btn.class" :btn-style="autoComputedStr.btn.style" @suffix-btn-click="$emit('suffix-btn-click')"></spf-bar><spf-bar v-if="iconRight && !isEmptyIcon" v-bind="autoComputed.iconbar.props" :icon-props="autoComputed.icon.props" :icon-class="autoComputedStr.icon.class" :icon-style="autoComputedStr.icon.style"></spf-bar></template><template v-else><spf-icon v-if="!iconRight && !isEmptyIcon" v-bind="autoComputed.icon.props" :root-class="autoComputedStr.icon.class" :root-style="autoComputedStr.icon.style"></spf-icon><spf-btn v-if="iconRight && !isEmptyBtn" v-bind="autoComputed.btn.props" :root-class="autoComputedStr.btn.class" :root-style="autoComputedStr.btn.style" @click="$emit('suffix-btn-click')"></spf-btn><slot v-bind="{ autoParams, cntProps: autoComputed.cnt.props, cntClass: autoComputed.cnt.class, cntStyle: autoComputed.cnt.style, }"></slot><spf-btn v-if="!iconRight && !isEmptyBtn" v-bind="autoComputed.btn.props" :root-class="autoComputedStr.btn.class" :root-style="autoComputedStr.btn.style" @click="$emit('suffix-btn-click')"></spf-btn><spf-icon v-if="iconRight && !isEmptyIcon" v-bind="autoComputed.icon.props" :root-class="autoComputedStr.icon.class" :root-style="autoComputedStr.icon.style"></spf-icon></template></div>`;
...
defineSpfBarSty.template = `<spf-ctn size="m" shape="sharp" tightness="normal" v-slot="{cntProps, cntClass, cntStyle}"><spf-bar icon="settings" :icon-props="{ mg: 'xl', mgPo: 'r' }" inner-content mg="xl" mg-po="t" v-bind="cntProps" :root-class="cntClass" :root-style="cntStyle"><spf-btn icon="settings" :icon-props="{ shape: 'sharp', icon: 'flag' }" label="设置" type="primary" closeable :hoverable="false"></spf-btn><spf-btn icon="settings" :icon-props="{ shape: 'sharp', icon: 'flag' }" label="设置" type="success" shape="pill" astitle></spf-btn><spf-btn icon="settings" label="设置" type="danger" active no-gap></spf-btn><spf-btn icon="settings" label="设置" type="warn" shape="sharp" no-gap></spf-btn><spf-btn icon="settings" label="设置" type="cyan" effect="fill"></spf-btn><spf-btn icon="settings" label="设置" stretch="grow"></spf-btn></spf-bar><spf-bar inner-content inner-content-rows mg="xl" mg-po="t" border="bd-m bd-po-all bdc-m" shape="round" v-bind="cntProps" :root-class="cntClass" :root-style="cntStyle" v-slot="{cntProps}"><spf-bar :size="cntProps.size" tightness="none" root-class="fc-l1 mg-m mg-po-r flex-x-end" root-style="width: 128px;">Label:</spf-bar><spf-ctn grow-in-bar inner-content v-bind="cntProps" root-style="width: 720px;"><spf-bar :size="cntProps.size" tightness="none"><el-input size="small"></el-input></spf-bar><spf-ctn :size="cntProps.size" border="bd-m bd-po-all bdc-m" shape="round" mg="xs" mg-po="y" inner-content><span>row 1</span><span>row 2</span><span>row 3</span><span>row 4</span></spf-ctn></spf-ctn></spf-bar><spf-bar><spf-ctn-vitem item-key="foooo" v-model="cvitemCollapse.a" icon="btn--message" label="折叠面板测试" info="这是标题更多信息" info-class="fs-s" border="bd-m"><div class="po" style="height: 128px;">content</div></spf-ctn-vitem></spf-bar><spf-bar><spf-ctn-vitem item-key="barrr" v-model="cvitemCollapse.b" size="medium" icon="btn--message" label="折叠面板测试" info="这是标题更多信息" info-class="fs-s" border="bd-m"><div class="po" style="height: 128px;">content</div></spf-ctn-vitem></spf-bar><spf-bar><spf-ctn-vitem item-key="jazzz" v-model="cvitemCollapse.c" icon="btn--message" label="折叠面板测试" info="这是标题更多信息" info-class="fs-s" border="bd-m"><div class="po" style="height: 128px;">content</div></spf-ctn-vitem></spf-bar></spf-ctn>`;
...
const defineSpfCtn = {
    ...
    props: {
        ...
        //innerContent == true 时，可以指定内部的包裹在 cnt 内容外的组件名称，默认 spf-ctn
        innerComponent: {
            type: String,
            default: 'spf-ctn'
        },
    ...
    data() {return {
        ...
        auto: {
            element: {
                root: {
                    class: 'spf-ctn block flex-y flex-x-start flex-no-shrink',
                },
                header: {
                    class: '',
                },
...
defineSpfCtn.template = `<div :class="autoComputedStr.root.class" :style="autoComputedStr.root.style"><template v-if="innerContent"><spf-bar v-if="withHeader" v-bind="autoComputed.header.props" :root-class="autoComputedStr.header.class" :root-style="autoComputedStr.header.style" @click="whenHeaderClick" v-slot="{autoParams: headerAutoParams}"><slot name="header" v-bind="{autoParams, headerAutoParams}"></slot></spf-bar><template v-if="withHeader || withFooter"><spf-ctn v-if="innerComponent==='spf-ctn'" v-bind="autoComputed.cnt.props" :root-class="autoComputedStr.cnt.class" :root-style="autoComputedStr.cnt.style" v-slot="{autoParams: cntAutoParams}"><slot v-bind="{autoParams, cntAutoParams}"></slot></spf-ctn><template v-else><component v-if="innerComponent!==''" :is="innerComponent" v-bind="autoComputed.cnt.props" :root-class="autoComputedStr.cnt.class" :root-style="autoComputedStr.cnt.style" v-slot="{autoParams: cntAutoParams}"><slot v-bind="{autoParams, cntAutoParams}"></slot></component><template v-else><slot v-if="!hideCnt" v-bind="{ autoParams, cntProps: autoComputed.cnt.props, cntClass: autoComputed.cnt.class, cntStyle: autoComputed.cnt.style }"></slot></template></template></template><template v-else><slot v-if="!hideCnt" v-bind="{ autoParams, cntProps: autoComputed.cnt.props, cntClass: autoComputed.cnt.class, cntStyle: autoComputed.cnt.style }"></slot></template><spf-bar v-if="withFooter" v-bind="autoComputed.footer.props" :root-class="autoComputedStr.footer.class" :root-style="autoComputedStr.footer.style" v-slot="{autoParams: footerAutoParams}"><slot name="footer" v-bind="{autoParams, footerAutoParams}"></slot></spf-bar></template><template v-else><template v-if="withHeader"><slot name="header" v-bind="{ autoParams, headerProps: autoComputed.header.props, headerClass: autoComputed.header.class, headerStyle: autoComputed.header.style }"></slot></template><slot v-if="!hideCnt" v-bind="{ autoParams, cntProps: autoComputed.cnt.props, cntClass: autoComputed.cnt.class, cntStyle: autoComputed.cnt.style }"></slot><template v-if="withFooter"><slot name="footer" v-bind="{ autoParams, footerProps: autoComputed.footer.props, footerClass: autoComputed.footer.class, footerStyle: autoComputed.footer.style }"></slot></template></template></div>`;

if (defineSpfCtn.computed === undefined) defineSpfCtn.computed = {};
defineSpfCtn.computed.profile = function() {return {};}
...
defineSpfCtnV.template = `<spf-ctn v-bind="autoComputed.root.props" inner-content v-slot="{autoParams: cntAutoParams, cntProps, cntClass, cntStyle}"><slot v-bind="{ autoParams: cntAutoParams, vProps: cntProps, vClass: cntClass, vStyle: cntStyle }"></slot></spf-ctn>`;

if (defineSpfCtnV.computed === undefined) defineSpfCtnV.computed = {};
defineSpfCtnV.computed.profile = function() {return {};}
...
const defineSpfCtnVitem = {
    ...
    props: {
        //当前折叠面板 item 在 spf-ctn-v 组件的面板列表中的唯一标记 key
        itemKey: {
            type: String,
            default: '',
            required: true
...
defineSpfCtnVitem.template = `<spf-ctn v-bind="autoComputed.root.props" inner-content with-header :header-props="autoComputed.header.props" :header-class="autoComputed.header.class" :header-style="autoComputed.header.style" :cnt-props="autoComputed.cnt.props" :cnt-class="autoComputed.cnt.class" :cnt-style="autoComputed.cnt.style" @header-click="$emit('change', !collapsed)"><template #header><spf-icon v-if="collapseSignLeft" v-bind="autoComputed.sign.props" :root-class="autoComputedStr.sign.class" :root-style="autoComputedStr.sign.style"></spf-icon><spf-icon v-if="$is.nemstr(icon)" v-bind="autoComputed.icon.props" :root-class="autoComputedStr.icon.class" :root-style="autoComputedStr.icon.style"></spf-icon><span v-if="$is.nemstr(label)" :class="autoComputedStr.label.class" :style="autoComputedStr.label.style">{{label}}</span><slot v-else name="label" v-bind="{ autoParams, labelProps: autoComputed.label.props }"></slot><span class="flex-1"></span><span v-if="$is.nemstr(info)" :class="autoComputedStr.info.class" :style="autoComputedStr.info.style">{{info}}</span><slot v-else name="info" v-bind="{ autoParams, infoProps: autoComputed.info.props }"></slot><spf-icon v-if="!collapseSignLeft" v-bind="autoComputed.sign.props" :root-class="autoComputedStr.sign.class" :root-style="autoComputedStr.sign.style"></spf-icon></template><template #default><slot v-bind="{ autoParams, cntProps: autoComputed.cnt.props, cntClass: autoComputed.cnt.class, cntStyle: autoComputed.cnt.style }"></slot></template></spf-ctn>`;

if (defineSpfCtnVitem.computed === undefined) defineSpfCtnVitem.computed = {};
defineSpfCtnVitem.computed.profile = function() {return {};}
...
'spf-btn': defineSpfBtn,
'spf-bar': defineSpfBar,
'spf-bar-sty': defineSpfBarSty,
'spf-ctn': defineSpfCtn,
'spf-ctn-v': defineSpfCtnV,
'spf-ctn-vitem': defineSpfCtnVitem,
