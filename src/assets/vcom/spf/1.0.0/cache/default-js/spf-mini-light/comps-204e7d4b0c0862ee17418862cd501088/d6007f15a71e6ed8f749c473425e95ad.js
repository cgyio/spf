...
defineSpfTabbar.template = `<div :class="styComputedClassStr.root" :style="styComputedStyleStr.root"><div class="tabbar-ctrl-start flex-x"><spf-button v-if="!noScroll" icon="keyboard-arrow-left" :size="ctrlSize" effect="popout" stretch="square" disabled></spf-button></div><div class="tabbar-list flex-1 flex-x flex-x-center flex-no-shrink"><div v-if="align !== 'left'" class="tabbar-list-holder"></div><div class="tabbar-item tabbar-item-active"><label>首页</label></div><div class="tabbar-item"><spf-icon icon="light-mode" type="warn"></spf-icon><label>关键页面</label></div><div class="tabbar-item"><label>设置</label></div><div class="tabbar-item"><label>页面标题很长的页面，这么长会不会有问题</label></div><div v-if="align !== 'right'" class="tabbar-list-holder"></div></div><div class="tabbar-ctrl-end flex-x"><spf-button v-if="!noScroll" :size="ctrlSize" icon="keyboard-arrow-right" effect="popout" stretch="square" disabled></spf-button></div></div>`;
