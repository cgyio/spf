...
const defineSpfBar = {
    ...
    data() {return {
        ...
        suffixBtnDefaultExtraSty: {
            ...
            style: [
                //iconRight == false
                'margin-right:-0.8em; margin-left:1.5em;',
                //iconRight == true
                'margin-left:-0.8em; margin-right:1.5em;',
            ],
        },
    }},
    computed: {
        ...
        //图标的额外样式
        iconExtraClass() {
            return this.mergeCustomStyleToDefault('class', 'icon');
        },
        iconExtraStyle() {
            return this.mergeCustomStyleToDefault('style', 'icon');
        },
        //行末按钮的额外样式
        suffixBtnExtraClass() {
            return this.mergeCustomStyleToDefault('class', 'suffixBtn');
        },
        suffixBtnExtraStyle() {
            return this.mergeCustomStyleToDefault('style', 'suffixBtn');
        },
    },
    methods: {
        /**
         * 合并 [icon|suffixBtn]DefaultSty.class|style 和 [icon|suffixBtn][Class|Style]
         * 根据 iconRight 状态决定最终合并结果
         */
        mergeCustomStyleToDefault(sty='class', elm='icon') {
            let is = this.$is,
                iss = s => is.string(s) && s!=='',
                isa = a => is.array(a) && a.length>0,
                iso = o => this.$cgy.isCssObj(o),
                tocss = o => this.$cgy.toCssString(o),
                mgcls = this.$cgy.mergeClassArr,
                mgcss = this.$cgy.mergeCss,
                ir = this.iconRight,
                dft = this[`${elm}DefaultExtraSty`][sty][ir?1:0],
                cst = this[`${elm}${sty.ucfirst()}`];
            //合并 class
            if (sty==='class') {
                if (!isa(dft) && !iss(dft)) dft = '';
                if (!iss(cst)) cst = '';
                return mgcls(dft, cst).join(' ');
            }
            //合并 style
            if (sty==='style') {
                if (!iss(dft) && !iso(dft)) dft = '';
                if (!iss(cst) && !iso(cst)) cst = '';
                return tocss(mgcss(dft, cst));
            }
            //默认
            return '';
        },
...
defineSpfBar.template = `<div :class="styComputedClassStr.root" :style="styComputedStyleStr.root"><spf-icon v-if="!iconRight && !isEmptyIcon" :icon="icon" v-bind="iconParams" :custom-class="iconExtraClass" :custom-style="iconExtraStyle"></spf-icon><spf-button v-if="iconRight && !isEmptyBtn" effect="popout" v-bind="suffixBtnParams" :icon="suffixBtn" :custom-class="suffixBtnExtraClass" :custom-style="suffixBtnExtraStyle" @click="$emit('suffix-btn-click')"></spf-button><slot :sty-props="styProps"></slot><spf-button v-if="!iconRight && !isEmptyBtn" effect="popout" v-bind="suffixBtnParams" :icon="suffixBtn" :custom-class="suffixBtnExtraClass" :custom-style="suffixBtnExtraStyle" @click="$emit('suffix-btn-click')"></spf-button><spf-icon v-if="iconRight && !isEmptyIcon" :icon="icon" v-bind="iconParams" :custom-class="iconExtraClass" :custom-style="iconExtraStyle"></spf-icon></div>`;
...
const defineSpfBlock = {
    ...
    props: {
        ...
        /**
         * 是否手动向默认插槽插入 spf-block 组件
         * 仅在 withHeader || withFooter 时生效
         * 手动插入 则不会在默认插槽外包裹 block 组件，由用户自行插入 spf-block  组件
         */
        innerBlock: {
            type: Boolean,
            default: false
    ...
    data() {return {
        ...
        sty: {
            init: {
                class: {
                    root: 'spf-block block flex-y flex-x-start',
                }
            },
            prefix: 'block',
            sub: {
    ...
    computed: {
        ...
        //是否在 grow == true 且 withHeader||withFooter 时 在 内部 block 组件上挂 scroll-y 类，返回：''|thin|bold
        innerBlockScroll() {
            let grow = this.grow,
                scr = this.scroll;
            if (!grow) return '';
...
defineSpfBlock.template = `<div :class="styComputedClassStr.root" :style="styComputedStyleStr.root"><template v-if="withHeader"><slot name="header" :sty-props="styProps"></slot></template><template v-if="(withHeader || withFooter)"><spf-block v-if="!innerBlock" :grow="grow" :scroll="innerBlockScroll"><slot :sty-props="styProps"></slot></spf-block><template v-else><slot :sty-props="styProps"></slot></template></template><template v-else><slot :sty-props="styProps"></slot></template><template v-if="withFooter"><slot name="footer" :sty-props="styProps"></slot></template></div>`;
