    ...
    methods: {
        ...
        async initMenus(menus=[]) {
            ...
            let is = this.$is,
                ...
                isa = a => is.array(a) && a.length>0,
                //iso = o => is.plainObject(o) && !is.empty(o),
                //ism = m => iso(m) && is.defined(m.key) && is.defined(m.label) && iss(m.key) && iss(m.label),
            ...
            //触发 menu-inited 事件 将处理后的 currentMenus 回传至外层组件
...
defineSpfMenu.template = `<spf-block v-bind="$attrs" :size="size" :grow="grow" :scroll="scroll" v-slot="{styProps, cntParams}"><template v-if="manual"><slot v-bind="{styProps, cntParams}"></slot></template><template v-if="!manual && inited && currentMenus.length>0"><template v-for="(menui, midx) of currentMenus"><spf-menu-item v-if="!menui.params.hide" :key="'spf-menu-item-'+midx" :size="styProps.size" :color="color" :background="background" :border="border" :label="menui.label" :icon="menui.icon" :sub="menui.sub" v-bind="menui.params" :menu-comp="$this" @menu-item-toggle-collapse="whenMenuItemToggleCollapse" @menu-item-click="whenMenuItemClick"></spf-menu-item></template></template></spf-block>`;
