...
const defineSpfBar = {
    ...
    props: {
        ...
        //border 边框样式
        /**
         * 容器类型组件 单独处理 border 边框样式
         *      false(默认)     不显示边框
         *      true            显示边框，默认 四边，bdc-m 颜色
         *                      可通过 bdPo|bdc 原子样式属性定义 边框位置|颜色
         */
        border: {
            type: Boolean,
            default: false
    ...
    data() {return {
        ...
        sty: {
            ...
            switch: {
                ...
                stretch:    '.bar-stretch stretch-{swv}',
...
defineSpfBar.template = `<div :class="styComputedClassStr.root" :style="styComputedStyleStr.root"><spf-icon v-if="!iconRight && !isEmptyIcon" :icon="icon" v-bind="iconParams" :custom-class="iconExtraClass" :custom-style="iconExtraStyle"></spf-icon><spf-button v-if="iconRight && !isEmptyBtn" effect="popout" v-bind="suffixBtnParams" :icon="suffixBtn" :custom-class="suffixExtraClass" :custom-style="suffixBtnExtraStyle" @click="$emit('suffix-btn-click')"></spf-button><slot></slot><spf-button v-if="!iconRight && !isEmptyBtn" effect="popout" v-bind="suffixBtnParams" :icon="suffixBtn" :custom-class="suffixExtraClass" :custom-style="suffixBtnExtraStyle" @click="$emit('suffix-btn-click')"></spf-button><spf-icon v-if="iconRight && !isEmptyIcon" :icon="icon" v-bind="iconParams" :custom-class="iconExtraClass" :custom-style="iconExtraStyle"></spf-icon></div>`;
