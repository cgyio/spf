...
cgy.def(Vue, {
     ...
     */
    service: {
        //此插件支持的 服务列表，必须对应着 mixin/service-base.js
        support: [
            //服务 必须严格按顺序加载
            'bus', 'ui', //'usr', 'nav', 'db',
        ],
...
bs.install = function(Vue, options = {}) {
    ...
    cgy.each(srvs, srv => {
        ...
        if (cgy.is.defined(srvo.init) && cgy.is(srvo.init, 'asyncfunction')) {
            ...
            Vue.service.initSequence.push(srvo.init.bind(srvo, opti));
        }
...
if (Vue.vcom.list.includes('spf')!==true) {
    Vue.vcom.list.push('spf');
    Vue.vcom.spf = {
        name: 'spf',
        isBase: true,
    }
