import PmsVcomComps from '/src/vcom/pms/1.0.0/async-default.js?create=true&ver=1.0.0&prefix=spf-sv&min=true';
...
let spaPages = {
    'spf-sv-pms-page-index': JSON.parse('{"isSpaPage":true,"spa":{"sort":1,"pageset":""}}'),
}
cgy.each(spaPages, (v,k) => {
    if (!Vue.service.options.nav.pages.includes(k)) {
        ...
        Vue.service.options.nav.page[k].spaPageName = 'spf-sv-pms-page-index';
    }
});
