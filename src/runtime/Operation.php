...
final class Operation 
{
    ...
    public static function initOprs($opt=[])
    {
         ...
         */
        //收集 全局路由 操作
        $initrut = self::initRoutes($opt["route"] ?? []);
        //收集 所有应用的 可用操作
        $initapp = self::initAppOprs();
        //收集 所有全局启用模块的 可用操作
        $initmod = self::initModuleOprs($opt["module"] ?? []);
    ...
    /**
     * 使用框架启动参数中 可能存在的 缓存文件路径 参数，覆盖当前类中定义的
     * @param Array $opt 框架启动参数
     * @return Bool
     */
    protected static function setCacheFile($opt=[])
    {
        if (!Is::nemarr($opt)) return false;
        //获取 $opt["operation"] 参数
        $oprc = $opt["operation"] ?? [];
        $cfs = $oprc["cache"] ?? [];
        $cf = $cfs["operation"] ?? null;
        $ccf = $cfs["closure"] ?? null;
        //配置 缓存文件位置
        if (Is::nemstr($cf)) self::$cacheFile = $cf;
        if (Is::nemstr($ccf)) self::$cacheClosureFile = $ccf;
        return true;
    }
    ...
    {
        //写入缓存
        Cache::save(self::$cacheFile, self::$context);
    ...
    {
        return Cache::isCached(self::$context);
    }
    ...
    protected static function initRoutes($opt=[])
    {
        ...
        foreach ($routes as $pattern => $oprc) {
            ...
            if (isset(self::$context[$appk]) && isset(self::$context[$appk][$oprn])) {
                //此路由操作 已在 应用的 操作列表中，仅修订 操作信息
                self::$context[$appk][$oprn]["route"] = $route;
                continue;
            }

            //操作写入对应的 应用操作列表
            $oprs = Arr::copy(self::$dftOprs);
    ...
    protected static function initAppOprs()
    {
        ...
        while(false!==($app = readdir($ph))) {
            ...
            //检查应用类是否存在
            if (Is::nemstr($appcls) && class_exists($appcls) && is_subclass_of($appcls, App::class)) {
                //应用名称 全小写下划线_ 形式
                $appk = Str::snake($appn,"_");
                //此应用的 所有可用操作
                $aoprs = Arr::copy(self::$dftOprs);
     ...
     * init 方法
     * 收集 模块的 可用操作
     * @param String|Array $mod 类全称  或  模块名  FooBar 或 foo_bar 形式，或者 类名数组，默认 null 
     * @return Bool
     */
    protected static function initModuleOprs($mod=null)
    {
        //未指定要收集的 模块，直接返回 true
        if (empty($mod)) return true;

        //批量收集多个模块，通常是在框架启动时，处理 全局启用的模块
        if (Is::nemarr($mod)) {
            if (Is::associate($mod)) {
                $mods = array_keys($mod);
            } else {
            ...
            //批量收集操作列表
            foreach ($mods as $mod) {
                $flag = $flag && self::initModuleOprs($mod);
            }
            return $flag;
        }

        //获取模块 类全称
        $modcls = Module::has($mod);
        if ($modcls === false) return false;

        //收集 类中定义的 特殊操作
        $apis = self::getClsOprs($modcls, "api");
        $views = self::getClsOprs($modcls, "view");
        $oprs = Arr::extend($apis, $views);
        
        //如果 App 已经实例化，则将所有 操作 都添加到 当前 App 名下
        if (App::$isInsed === true) {
        ...
        //App 还未实例化，则将操作列表写入 所有 App 名下
        //写入
        $appks = array_keys(self::$context);
        foreach ($appks as $appk) {
            if (App::has($appk)!==false) {
                self::setAppOprs($appk, $oprs);
            }
        }
    ...
    public static function getAppOprs($appcls, $oprs=null)
    {
        ...
        //遍历 类中定义的 特定方法
        //api 方法
        $apis = self::getClsOprs($appcls, "api", "public,&!static", $appk, $appintr);
        //view 方法
        $views = self::getClsOprs($appcls, "view", "public,&!static", $appk, $appintr);
        //合并
        $oprs = Arr::extend($oprs, $apis, $views);
        //返回
        return $oprs;
        /*$apis = Cls::specific(
            $appcls,
    ...
    public static function setAppOprs($app, $oprs=[])
    {
        ...
        //当前已保存的 应用操作列表
        $aoprs = self::$context[$appk] ?? [];

        //如果未指定要合并的 数据，直接返回已保存的 操作列表
        if (!Is::nemarr($oprs)) return $aoprs;
        if (!Is::nemarr($aoprs)) {
            //如果当前没有已保存的 操作列表，直接写入
            self::$context[$appk] = $oprs;
            return $oprs;
        }

        //合并 
        //标准 操作列表格式
        $oprs = Arr::extend(self::$dftOprs, $oprs);
        self::$context[$appk] = Arr::extend($aoprs, $oprs);

        return self::$context[$appk];
    }
    ...
    {
        //Operation 类必须已初始化
        if (self::$isInited!==true || !Is::nemarr(self::$context)) return [];
    ...
    /**
     * 解析某个类中定义的 api|view 方法，生成操作信息数组
     ...
     */
    protected static function getClsOprs($cls, $type="api", $filter="public,&!static", $pre=null, $intr=null)
    {
        //准备标准的 操作列表输出数据
        $oprs = Arr::copy(self::$dftOprs);
        //传入的类 不存在
        if (!class_exists($cls)) return $oprs;
        ...
        if (!Is::nemstr($pre)) {
            //从 $cls 类全称 解析 操作标识前缀
            if (is_subclass_of($cls, Core::class) === true) {
                //这是 核心类
                $clsk = $cls::clsk();
                if (is_subclass_of($cls, App::class) === true) {
                    //针对 应用类
                    $pre = $clsk;
                } else if (is_subclass_of($cls, Module::class) === true) {
                    //针对 模块类
                    $pre = "module/$clsk";
                } else {
                    //针对其他核心类
                ...
                //其他类 使用 类全称路径形式，作为 操作标识前缀
                $clsarr = explode("\\", trim($cls, "\\"));
                $clsarr = array_map(function($cli) {
    ...
    public function __construct($app)
    {
        ...
        $this->app = $app;
        //从 Operation::$context 中读取对应 app 的相关操作
        $appk = $app::clsk();
        $ctx = self::$context;
        if (isset($ctx[$appk])) {
