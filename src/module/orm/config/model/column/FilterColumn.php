 ...
 * SPF-Orm 数据库处理模块
 * 数据库配置解析工具类
 * 专门解析 filter 特殊类型的 工具类
 * 
 * 处理数据库配置参数中的  $conf["model"]["model_name"]["column"]["filter"] 项目
 */
...
class FilterColumn extends SpecialColumn 
{
    //引用 trait
    ...
    public function parse()
    {
        ...
        //增加前端参数中的 缓存标记，用于前端操作时，缓存每个可筛选字段的 筛选参数
        $rtn["front"] = [
            "table" => [
                "cache" => [
                    "filter" => []
        ...
        foreach ($this->origin as $colk) {
            //写入默认的 筛选参数，前端修改某个字段的筛选参数后，将会缓存在前端
            //!! 此处指定的默认筛选参数，会被前端缓存的 覆盖掉
            $rtn["front"]["table"]["cache"]["filter"][$colk] = [
                "logic" => "",
                "value" => null
            ];
        }
