 ...
 * SPF-Orm 数据库处理模块
 * 数据库配置解析工具类
 * 专门解析 search 特殊类型的 工具类
 * 
 * 处理数据库配置参数中的  $conf["model"]["model_name"]["column"]["search"] 项目
 */
...
class SearchColumn extends SpecialColumn 
{
    //引用 trait
    ...
    public function parse()
    {
        ...
        //!! 不启用 search 关键字的 前端缓存
        //增加前端参数中的 缓存标记，用于前端操作时，缓存每个可搜索字段的 搜索参数
        $rtn["front"] = [
            "table" => [
                "cache" => [
                    "search" => []
        ...
        foreach ($this->origin as $colk) {
            //写入默认的 搜索参数，前端修改某个字段的搜索参数后，将会缓存在前端
            //!! 此处指定的默认搜索参数，会被前端缓存的 覆盖掉
            $rtn["front"]["table"]["cache"]["search"][$colk] = [
                "keyword" => "",
            ];
        }
