 ...
 * SPF-Orm 数据库处理模块
 * 数据库配置解析工具类
 * 专门解析 generator 特殊类型的 工具类
 * 
 * 处理数据库配置参数中的  $conf["model"]["model_name"]["column"]["generator"] 项目
 */
...
class GeneratorColumn extends SpecialColumn 
{
    //引用 trait
    ...
    public function parse()
    {
        ...
        //向 字段参数中插入 generator 参数
        if (!isset($rtn["column"]) || !Is::associate($rtn["column"])) $rtn["column"] = [];
        foreach ($this->origin as $colk) {
            $rtn["column"][$colk] = [
                //generator 参数是 可以直接传入 call_user_func 的 callable[]
                "generator" => [
                    //当前 数据模型的 类全称
                    $this->params->model,
                    //generator 方法名 fooBarGenerator
                    Str::camel($colk)."Generator",
                ]
            ];
        }
