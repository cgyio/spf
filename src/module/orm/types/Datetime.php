...
class Datetime extends Types 
{
     ...
     */
    protected static $optProps = ["datetime"];
    ...
    protected static function parseDefaultValue($defv=null, $defc=null)
    {
        ...
        //传入的默认值 是 整数 或 整数字符串
        if (is_numeric($defv) && is_int($defv*1)) {
            return [
                "value" => $defv*1,
                "insql" => (string)$defv,
            ];
        ...
        //传入的默认值 是 日期字符串 Y-m-d h:i:s
        if (Dater::isStr($defv)) {
            if ($deftp==="datetime") {
                //datetime 类型 默认使用 09 点的时间戳
                if (strpos($defv, " ")===false) $defv .= " 09:00:00";
            }
            $dvi = strtotime($defv);
            return [
                "value" => $dvi,
     ...
     */
    //获取此刻时间戳
    public static function defaultNowGetter($colk, $conf=[])
    {
        $tp = $conf["datetime"]["type"] ?? static::$optDefineTypes[0];
        $t = time();
        if ($tp==="datetime") return $t;
        $ts = date("Y-m-d", $t);
        return strtotime($ts);
    }
    //获取今天的 0 点时间戳
    public static function defaultTodayGetter($colk, $conf=[])
    {
        $ts = date("Y-m-d", time())." 00:00:00";
        return strtotime($ts);
    }
    //获取今天的 23:59:59 的时间戳
    public static function defaultTodayEndGetter($colk, $conf=[])
    {
    ...
    {
        if (!is_numeric($val) || !Dater::isTs($val)) return null;
        return $val * 1;
    }
     ...
     * @type String
     * @jstype String
     * @param Int $val 实际字段的值 时间戳
     * @return String|null 转换为对应时间字符串
     */
    public function strGetter($val)
    {
        ...
        //类型参数
        $tp = $this->opt("type");
        return $tp==="datetime" ? date("Y-m-d H:i:s", $val) : date("Y-m-d", $val);
    }
